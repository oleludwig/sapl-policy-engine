...
public final class FunctionUtil {
    public static Flux<Val[]> combineArgumentFluxes(Arguments arguments) {
        if (arguments == null || arguments.getArgs().isEmpty())
            return Mono.just(new Val[0]).flux();
        return combine(argumentFluxes(arguments));
    }

    public static String resolveAbsoluteFunctionName(Iterable<String> steps, Map<String, String> imports) {
        var functionName = mergeStepsToName(steps);
        return imports.getOrDefault(functionName, functionName);
    }

    public static String resolveAbsoluteFunctionName(String unresolvedFunctionName, Map<String, String> imports) {
        return imports.getOrDefault(unresolvedFunctionName, unresolvedFunctionName);
    }

    public static Mono<Val> evaluateFunctionMono(Iterable<String> fsteps, Val... parameters) {
        return evaluateFunctionMono(mergeStepsToName(fsteps), parameters);
    }

    public static Mono<Val> evaluateFunctionMono(String unresolvedFunctionName, Val... parameters) {
    ...
    public static Mono<Val> evaluateFunctionWithLeftHandArgumentMono(Iterable<String> fsteps, Val leftHandArgument,
            Val... parameters) {
    ...
    private static Stream<Flux<Val>> argumentFluxes(Arguments arguments) {
        return arguments.getArgs().stream().map(Expression::evaluate);
    }

    private static Flux<Val[]> combine(Stream<Flux<Val>> argumentFluxes) {
        List<Flux<Val>> x = argumentFluxes.toList();
        return Flux.combineLatest(x, e -> Arrays.copyOf(e, e.length, Val[].class));
    }

    private static String mergeStepsToName(Iterable<String> steps) {
